...
export function Navbar() {
  ...
  const getRoleDisplay = () => {
    ...
    switch (user.role) {
      ...
      default: return { label: getInitials(user.name), icon: UserRound, color: "text-slate-400", bg: "bg-slate-800" };
    }
  };
              ...
              <div className="relative flex items-center gap-2 sm:gap-3" ref={dropdownRef}>
                ...
                {dropdownOpen && (
                  <div className="absolute right-0 top-full mt-2 w-48 rounded-md border border-border/70 bg-card p-1 shadow-md">
                    {!isResponder && (
                      <>
                        <Link href="/profile" className="flex items-center gap-2 rounded-sm px-3 py-2 text-sm hover:bg-secondary" onClick={() => setDropdownOpen(false)}>
                          <UserRound className="size-4 text-muted-foreground" /> My Profile
                        </Link>
                        <Link href="/dashboard" className="flex items-center gap-2 rounded-sm px-3 py-2 text-sm hover:bg-secondary" onClick={() => setDropdownOpen(false)}>
                          <FileText className="size-4 text-muted-foreground" /> My Reports
                        </Link>
                        <Link href="/community-risk" className="flex items-center gap-2 rounded-sm px-3 py-2 text-sm hover:bg-secondary" onClick={() => setDropdownOpen(false)}>
                          <Activity className="size-4 text-muted-foreground" /> CRI
                        </Link>
                        <Link href="/settings" className="flex items-center gap-2 rounded-sm px-3 py-2 text-sm hover:bg-secondary" onClick={() => setDropdownOpen(false)}>
